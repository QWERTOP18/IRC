...
ResponseBody Join::run(int t_fd, RequestBody t_request)
{
    ...
    Channel *ch = m_Model->getChannel(id_hash(t_request.m_target_channel));
    if (ch != NULL)
    {
        if (m_Model->isClientOnChannel(t_fd, ch->getId()))
        {
            response.m_status = -1; // ERR_ALREADYONCHANNEL; ✨update
            return response;
        }

        m_Model->addHub(t_request.m_fd, id_hash(t_request.m_target_channel), MEMBER);
        response.m_status = RPL_TOPIC;
        // response.m_content = "No topic is set";
        this->broadcast(t_fd, m_Model->getChannel(id_hash(t_request.m_target_channel))->getId(), "user joined");
        return response;
    }
    ...
    m_Model->addHub(t_request.m_fd, id_hash(t_request.m_target_channel), ADMIN);
