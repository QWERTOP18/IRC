...
{
    EXPECT_THROW({ MockServer server("invalid", "Abcd", model, controller); }, std::invalid_argument);
}

TEST_F(ServerTest, Initialization_InvalidPort_TooSmall)
{
    EXPECT_THROW({ MockServer server("-1", "Abcd", model, controller); }, std::invalid_argument);
}
TEST_F(ServerTest, Initialization_InvalidPort_TooLarge)
{
    EXPECT_THROW({ MockServer server("70000", "Abcd", model, controller); }, std::invalid_argument);
}

TEST_F(ServerTest, Initialization_EmptyPassword)
{
    EXPECT_THROW({ MockServer server("6666", "", model, controller); }, std::invalid_argument);
}
TEST_F(ServerTest, Initialization_InvalidPassword_TooShort)
{
    EXPECT_THROW({ MockServer server("6666", "abc", model, controller); }, std::invalid_argument);
}
...
{
    EXPECT_THROW({ MockServer server("6666", "ABCD", model, controller); }, std::invalid_argument);
}
